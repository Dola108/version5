...
while($num_row!=0) {
  ...
  $n=$rw['id'];
  $post = preg_replace( '/(https{0,1}:\/\/[\w\-\.\/#?&=]*)/', "<a href='$1' target='_blank' style='text-decoration:none; font-size: 14px; color: #eaeaea; font-family: sans-serif;'>$1</a>", $rw['texts'] );
      ...
      $.ajax ({
        ...
        success:function(response) {
          if(response=="success") {
              ...
              $("#delt-"+x).html("<a style='font-size:12px;color:red;'>deleted</a>");
          }
        }
      });
    });
